...
(function (global) {
  ...
  var STATE = {
    initialized: false,
    rosterSheetsData: null, // { sheetName: rows2D }
    amazonRows: null,
    ...
    diagnostics: { undefinedCodeCounts: {}, warnings: [] },
  };
  ...
  function ensureRendered() {
    ...
    container.innerHTML =
      ...
      '  <div class="card p-4">' +
      '    <h3 class="text-sm font-bold mb-2">① 月間シフト表（高橋さんExcel）読込</h3>' +
      '    <p class="text-xs text-ink-lighter mb-2">DSP別シート（GDS等）からTransport ID・勤務記号を抽出します</p>' +
      '    <input type="file" id="rolling-file-roster" accept=".xlsx,.xls" class="hidden">' +
      '    <button id="rolling-btn-roster" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-bold hover:bg-blue-700">📂 ファイル選択</button>' +
  ...
  function handleRosterFile(evt) {
    ...
    reader.onload = function (e) {
      try {
        ...
        STATE.rosterSheetsData = sheetsData;
        statusEl.innerHTML = '<span class="text-green-400">✓ ' + scannedSheets.length + 'シート読込（' + scannedSheets.join(', ') + '）</span>';
        readOptionsFromUI();
        recomputeAndRender();
      } catch (ex) {
  ...
  function recomputeAndRender() {
    if (!STATE.rosterSheetsData && !STATE.amazonRows) return;
    ...
    // 1) ロースター（複数DSPシート）を解析・統合
    var driverMeta = {}; // key -> {name, company, transportId}
    var rosterBlocksByDriver = {}; // key -> {date: blocks[]}
    if (STATE.rosterSheetsData) {
      Object.keys(STATE.rosterSheetsData).forEach(function (sheetName) {
        var rows = STATE.rosterSheetsData[sheetName];
        ...
        warnings = warnings.concat(
          parsed.warnings.map(function (w) {
            return Object.assign({ sheet: sheetName }, w);
          })
        );
        var grouped = RollingCore.groupRosterRecordsByDriver(parsed.records);
        Object.keys(grouped).forEach(function (key) {
          driverMeta[key] = grouped[key].meta;
          rosterBlocksByDriver[key] = rosterBlocksByDriver[key] || {};
          var src = grouped[key].blocksByDate;
          Object.keys(src).forEach(function (date) {
            rosterBlocksByDriver[key][date] = (rosterBlocksByDriver[key][date] || []).concat(src[date]);
            src[date].forEach(function (b) {
              if (!b.defined) {
    ...
    STATE.diagnostics = { undefinedCodeCounts: undefinedCodeCounts, warnings: warnings };
  ...
  function renderGrid() {
    ...
    STATE.drivers.forEach(function (driver) {
      ...
      html += '<td class="p-2 sticky left-0 bg-inherit font-medium">' + driver.meta.name + (overWindowCount > 0 ? ' <span class="text-red-400 text-[10px]">(超過期間' + overWindowCount + '件)</span>' : '') + '</td>';
  ...
  var RollingUI = {
    onShown: function () {
      ensureRendered();
      if (STATE.rosterSheetsData || STATE.amazonRows) {
        recomputeAndRender();
      }
    },
