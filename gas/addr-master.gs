...
function deduplicateAddresses() {
  ...
  var values = sheet.getRange(2, 1, lastRow - 1, 4).getValues();
  var originalCount = values.length;
  var unique = {};
  for (var i = 0; i < values.length; i++) {
    var addr = String(values[i][0] || '').trim();
    if (!addr) continue;
    var key = normalizeAddress(addr);
    var hasCoord = values[i][1] !== '' && values[i][2] !== '';
    if (!unique[key] || hasCoord) {
      unique[key] = [addr, values[i][1], values[i][2], values[i][3]];
    }
  }
  ...
  return { removed: originalCount - rows.length, kept: rows.length };
}
